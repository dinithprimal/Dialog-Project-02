<?php
    ...
    $CXOconfirmProg = mysqli_fetch_array($cnfrmId);

    $querySelCXO = "SELECT * FROM cxoapproval WHERE idProgram = '$id' AND CXOid = '$unit'";
    $cnfrmIdSelCXO = mysqli_query($conn,$querySelCXO) or die(mysqli_error($conn));
    $CXOis = mysqli_num_rows($cnfrmIdSelCXO);
    $CXOconfirm = mysqli_fetch_array($cnfrmIdSelCXO);
    $CXOStage = $CXOconfirm['progApprXCO1'];
...
<html lang="en">
    ...
    <body>
        <div class="container-fluid bg">
            <?php
                if($CXOis==1 & $CXOconfirm['CXO1Confirm'] == $cnfrm){
                    if($CXOStage == '1'){

                        $updateReject = "UPDATE cxoapproval SET progApprCXO1= '3', dtApprCXO1 = '$Dt' WHERE idProgram = '$id' AND CXOid = '$unit'";

                        if ($conn->query($updateReject) === TRUE) {

                            $updateRejectProg = "UPDATE programstatus SET statusCXO1 = '3'  WHERE idProgram = '$id'";
                            if ($conn->query($updateRejectProg) === TRUE) {

                            }

                            echo    '<script type="text/javascript"> 
                                        function sendMailRejectCXO(){
                                            var id = '.$id.';
                                            $.ajax({
                                                url: "sendMailRejectCXO.php",
                                                method: "POST",
                                                data: {id:id},
                                                dataType: "text",
            ...
            ?>
                ...
                <div class="row mt-5 pt-5">            
                    ...
                    <div class="col-xl-6 col-lg-6 col-md-8 col-sm-12 col-xs-12">
                        <div class="row">
                            <div class="col">
                                <h1 class="text-center"><span class="badge badge-pill badge-danger">Rejected</span></h1>
                            </div>
                        </div>
                        <div class="row mt-5">
                            <div class="col">
                                <h4 class="text-center"><small>You have rejected <b><?php  echo    "".$CXOconfirmProg['progName']." ".$CXOconfirmProg['progType']." ";    ?></b>training program</small></h4>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-2 col-sm-12 col-xs-12"></div>
