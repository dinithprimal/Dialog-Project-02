        ...
        <div class="container-fluid bg">
            <?php
                if($CXOis==1 & $CXOconfirm['CXO1Confirm'] == $cnfrm){
                    if($CXOStage == '1'){
                        $updateAccept = "UPDATE cxoapproval SET progApprCXO1= '2', dtApprCXO1 = '$Dt' WHERE idProgram = '$id' AND CXOid = '$unit'";
                        if ($conn->query($updateAccept) === TRUE) {
                            ...
                            while($CXOconfirm = mysqli_fetch_array($cnfrmIdSelCXO)){
                                if($CXOconfirm['progApprCXO1'] != '2'){
                                    $allCnf = 'false';
                                }
                            }
                            if($allCnf == 'true'){
                                $updateAcceptProg = "UPDATE programstatus SET statusCXO1 = '2'  WHERE idProgram = '$id'";
                                if ($conn->query($updateAcceptProg) === TRUE) {

                                }
                            }

                            echo    '<script type="text/javascript"> 
                                        function sendMailAproveCXO(){
                                            var id = '.$id.';
                                            $.ajax({
                                                url: "sendMailAproveCXO.php",
                                                method: "POST",
                                                data: {id:id},
                                                dataType: "text",
            ...
            ?>
                ...
                <div class="row mt-5 pt-5">            
                    ...
                    <div class="col-xl-6 col-lg-6 col-md-8 col-sm-12 col-xs-12">
                        ...
                        <div class="row mt-5">
                            <div class="col">
                                <h4 class="text-center"><small>You have succesfully accepted <b><?php  echo    "".$CXOconfirmProg['progName']." ".$CXOconfirmProg['progType']." ";    ?></b>training program</small></h4>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-2 col-sm-12 col-xs-12"></div>
            ...
            <?php
                            if($CXOconfirm['progApprGTD'] == '2'){
                                if($allCnf == 'true'){
                                    echo    '<script type="text/javascript">
                                                function sendMailGCTO(){
                                                    var id = '.$id.';
                                                    $.ajax({
